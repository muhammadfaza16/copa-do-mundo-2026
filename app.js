...
function updateHeroPanel() {
      ...
      cdDisplay.innerHTML = `
        <div class="live-scoreboard">
          <div class="live-info-wrapper" style="display: flex; flex-direction: column; align-items: center; gap: 4px; width: 100%;">
            ...
            <!-- Row 2: Grup + Venue -->
            <div class="live-venue-row-styled" style="text-align: center; margin-top: 10px; margin-bottom: 2px;">
  ...
  if (diff <= 0) {
    ...
    if (lastHeroMatchKey !== cdKey) {
      ...
      subEl.innerHTML = `
        <div style="font-size: 0.62rem; font-weight: 700; color: var(--primary-gold); margin-top: 4px; margin-bottom: 3px; letter-spacing: 0.5px;">
          ${stageName}
        </div>
...
function recalculateKnockoutTree() {
  ...
  sortedKO.forEach(m => {
    ...
    // Auto-resolve winner if a finished score exists in realScores
    if (!winner) {
      const scoreKey = `ko_${m.match_id}`;
      const scoreData = realScores[scoreKey];
      if (scoreData && scoreData.status === 'FINISHED') {
        if (scoreData.score1 !== null && scoreData.score2 !== null) {
...
const BASE_COMPACT_COORDINATES = {
  ...
  73: { x: 30, y: 225 },  // Column 1 upper top (aligned for fork center)
  75: { x: 30, y: 305 },  // Column 1 upper bottom (aligned for fork center)
  83: { x: 30, y: 415 },  // Column 1 lower top (aligned for fork center)
  84: { x: 30, y: 495 },  // Column 1 lower bottom (aligned for fork center)
  
  89: { x: 290, y: 182 }, // Stack Top R16
  90: { x: 172, y: 265 }, // Column 2 Upper R16 (centered & balanced)
  97: { x: 290, y: 265 }, // Stack QF Upper (balanced)
  ...
  93: { x: 172, y: 455 }, // Column 2 Lower R16 (centered & balanced)
  94: { x: 290, y: 538 }, // Stack Bottom R16
  98: { x: 290, y: 455 }, // Stack QF Lower (balanced)
  ...
  79: { x: 846, y: 225 }, // Column 7 upper top (aligned for fork center)
  80: { x: 846, y: 305 }, // Column 7 upper bottom (aligned for fork center)
  
  91: { x: 586, y: 182 }, // Stack Top Right R16
  92: { x: 704, y: 265 }, // Column 6 Upper R16 (centered & balanced)
  99: { x: 586, y: 265 }, // Stack QF Upper Right (balanced)

  // Bottom Wing (Column 7 lower R32, Bottom-right horizontal R32)
  86: { x: 846, y: 415 }, // Column 7 lower top (aligned for fork center)
  88: { x: 846, y: 495 }, // Column 7 lower bottom (aligned for fork center)
  85: { x: 496, y: 640 }, // Bottom-right horizontal left (Swiss vs 3rd)
  87: { x: 676, y: 640 }, // Bottom-right horizontal right (Juara Grup K vs 3rd)
  
  95: { x: 704, y: 455 }, // Column 6 Lower R16 (centered & balanced)
  96: { x: 586, y: 538 }, // Stack Bottom R16 Right
  100: { x: 586, y: 455 }, // Stack QF Lower Right (balanced)
...
function getMatchTooltipHtml(m) {
  ...
  let winReason = '';
  if (apiMatchData && apiMatchData.status === 'FINISHED') {
    const s1 = parseInt(apiMatchData.score1);
    const s2 = parseInt(apiMatchData.score2);
    const hasShootout = apiMatchData.shootout_score1 !== null && 
                        apiMatchData.shootout_score1 !== undefined && 
                        apiMatchData.shootout_score2 !== null && 
                        apiMatchData.shootout_score2 !== undefined;
    
    const statusName = apiMatchData.espn_status_name || '';
    const statusDetail = apiMatchData.espn_status_detail || '';
    const periodDesc = apiMatchData.period_desc || '';
    const clock = apiMatchData.display_clock || '';
...
function renderBracketSchedule() {
  ...
  let mappedKnockout = knockoutMatches.map(m => {
    const matchKey = `ko_${m.match_id}`;
    const score = getMatchScore(matchKey);
    if (!score && !showPotentialDraw && !isGroupStageComplete()) {
      if (!isRealTeamName(m.team1) || !isRealTeamName(m.team2)) {
  ...
  if (activeScheduleRound === 'Finals') {
    filtered = mappedKnockout.filter(m => m.group === 'Third-place match' || m.group === 'Final');
  } else {
    filtered = mappedKnockout.filter(m => m.group === activeScheduleRound);
  }
...
async function fetchScoresDirectFromEspn() {
  ...
  const KNOCKOUT_MAPPING = {
    ...
    "760501": 87, "760502": 90, "760503": 89, "760504": 91, "760505": 92,
    "760506": 93, "760507": 94, "760508": 96, "760509": 95, "760510": 97,
